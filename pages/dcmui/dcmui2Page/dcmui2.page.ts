...
export class Dcmui2Page {
  ...
  private readonly searchResultOption: Locator;
  private readonly addressTab: Locator;
  private readonly addressRowButton: Locator;
  ...
  constructor(page: Page) {
    ...
    this.searchResultOption = page.getByRole('option', { name: 'ANGULARYIRMI INDIVIDUAL 2007/' });
    this.addressTab = page.getByRole('tab', { name: 'Address' });
    this.addressRowButton = page.getByRole('row', { name: 'DEFAULT_BILLING_ADDRESS ASD' }).getByRole('button');
  ...
  public async clickElement(elementIdentifier: string): Promise<void> {
    switch (elementIdentifier) {
      ...
      case 'Address':
        ...
        break;
      case 'DEFAULT_BILLING_ADDRESS ASD':
        // Menu Trigger Pattern: Wait for the button in the row, click it, and wait for the menu to appear.
        await this.addressRowButton.waitFor({ state: 'visible', timeout: 10000 });
        await this.addressRowButton.click();
        await this.page.waitForTimeout(500);
        await this.page.waitForSelector('[role="menu"], [role="menuitem"], mat-menu, .mat-menu-panel', { state: 'visible', timeout: 5000 }).catch(() => {});
        break;
      case 'Edit':
        // Menu Item Pattern: Wait for the menu item to be visible, then click. This also triggers a modal.
        await this.editMenuitem.waitFor({ state: 'visible', timeout: 15000 });
        ...
        break;
      case 'Address Line 2':
        // This field appears in a modal, so add a wait.
  ...
  public async fillField(fieldIdentifier: string, value: string): Promise<void> {
    switch (fieldIdentifier) {
      ...
      case 'Type text..':
        ...
        break;
      case 'Address Line 2':
        // Form Field After Action Pattern: Wait for field to be ready as it's in a modal.
        await this.addressLine2Textbox.waitFor({ state: 'visible', timeout: 8000 });
        await this.page.waitForTimeout(150);
